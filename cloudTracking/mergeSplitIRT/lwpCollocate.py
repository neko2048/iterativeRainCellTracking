...
class Collocator:
    ...
    def lwpCollocate(self, lwpTrackField, lwpTrackMetaData, trackLinkList, trackIDRecord, findGridSize=2):
        """
        INPUT: 
            lwpTrackField: (y, x)
        ...
        for _, eachObject in lwpTrackMetaData.iterrows():
            ...
            mappedTrackField[lwpTrackField==eachObject["objectID1"]] = trackLinkList[trackIDRecord == eachObject["trackID"]][0]
...
if __name__ == "__main__":
    ...
    outputDir = homePath + "transition/dat/reindexLWP/reindexLWP-25e-3/"
    minPerTimeIdx = 10
    ...
    for tIdx in timeIdxArange:
        ...
        lwpTrackField = collocator.lwpCollocate(lwpTrackField, lwpTrackMetaData, trackLinkList, trackIDRecord)
        ...
        #pcolormesh(xc/1e3, yc/1e3, cloudObject)
        #pcolormesh(xc/1e3, yc/1e3, np.ma.masked_array(lwpTrackField%23, lwpTrackField<=0), cmap="rainbow", shading="nearest", vmin=0, vmax=19)
        ...
        #    text(i["weightCenX"]*1e2/1e3, i["weightCenY"]*1e2/1e3, s=int(trackLinkList[trackIDRecord == i["trackID"]][0]), color='black', fontsize=15)
        #xlim(0, 102.4)
        #ylim(0, 102.4)
