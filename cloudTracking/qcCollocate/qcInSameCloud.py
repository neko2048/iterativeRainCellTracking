...
if __name__ == "__main__":
    ...
    qcObjectPath = homePath + "transition/dat/cloudLabel/qc2LayerFilter/"
    ...
    collocateQcOutputPath = homePath + "transition/dat/cloudLabel/qc2LayerFilterInSameCloud/"
    minPerTimeIdx = 10
    ...
    #trackIDList = np.arange(1, np.max(trackOutputData["trackID"])+1, 1).astype(int)
    ...
    for tIdx in timeIdxArange:
        ...
        newQcObject = np.zeros(cloudObject.shape)
        #collocateQc = np.zeros(shape=(len(zc), len(yc), len(xc)))
        if np.max(cloudObject) > 0:
            ...
            newQcObject = np.zeros(qcObject.shape)
        
            for cloudIdx in cloudObjectCandicate:
                print("Cloud: {} / {}".format(cloudIdx, cloudObjectCandicate[-1]))
                newQcObject[np.logical_and(qcObject != 0, cloudObject == cloudIdx)] = np.max((cloudObject==cloudIdx) * qcObject)
                ...
                #    continue
                #maxQcIdx = np.max(correspondQc)
                #print(correspondQc, " to ", maxQcIdx)
                #for qcIdx in correspondQc:
                #    newQcObject[qcObject == qcIdx] = maxQcIdx
        #for qcIdx in qcObjectCandicate:
        #    print("Qc: {} / {}".format(qcIdx, qcObjectCandicate[-1]))
        #    if qcIdx not in newQcObject: 
        #        print("This Qc has been overwritten")
        #        continue
        #    correspondCloud = np.unique(cloudObject * (newQcObject==qcIdx))
        #    correspondCloud = correspondCloud[correspondCloud > 0]
        #    print("Correspond Cloud: {}".format(correspondCloud))
        #    connectQc = np.unique(newQcObject * (cloudObject == correspondCloud))
        #    connectQc = connectQc[connectQc > 0]
        #    print("Connected Qc: ", connectQc)
        #    for i in connectQc:
        #        newQcObject[qcObject == i] = qcIdx
        ...
        newQcObject = newQcObject[np.newaxis, :, :, :] # same dim with VVM.
        xrCollocateQcObject = xr.DataArray(newQcObject.astype(int), 
